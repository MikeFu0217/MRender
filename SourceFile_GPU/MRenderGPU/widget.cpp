    ...
    ui(new Ui::Widget)
{
    ...
    ui->label_alpha->setText(QString("alpha=")+QString::number(alpha,'f',2));
    ui->label_beta->setText(QString("beta=")+QString::number(beta,'f',2));
    ui->label_rho->setText(QString("rho=")+QString::number(rho,'f',2));
    ...
    ui->horizontalSlider_pho->setEnabled(false);
    //GPU is on
...
void Widget::loadVideo2Array3D()
{
    ...
    //show success
    QMessageBox::information(this,"Reminder","Image sequence loaded!");
}
void Widget::on_loadFileButton_clicked()
{
    ...
    ui->horizontalSlider_pho->setEnabled(true);
    //get filePath
    ...
    programmeIsOn = true;
    //GPU is on
    this->GPUisON = test_cuda();
    if(this->GPUisON==0){
        ui->checkBox_2->setChecked(false);
        ui->checkBox_2->setText("GPU OFF");
        ui->checkBox_2->setEnabled(false);
    }else{
        ui->checkBox_2->setChecked(true);
        ui->checkBox_2->setText("GPU ON");
...
void Widget::show3D()
{
    ...
    //trace ray
    if(this->GPUisON){
        traceRayGPU(cvs.data,
                    cvs.get_height(), cvs.get_width(),
                    viewport.get_width(), viewport.get_height(),
                    viewport.center.get_x(),viewport.center.get_y(),viewport.center.get_z(),this->beta,
                    viewport.camera.get_x(),viewport.camera.get_y(),viewport.camera.get_z(),
                    //intercubes
                    scene.getCube().xScale, scene.getCube().yScale, scene.getCube().zScale,
                    scene.getCube().xlength, scene.getCube().ylength, scene.getCube().zlength, scene.getCube().maxlength,
                    scene.getCube().xshift, scene.getCube().yshift, scene.getCube().zshift,
                    scene.getCube().center.x, scene.getCube().center.y, scene.getCube().center.z,
                    scene.getCube().XOY_z_pos, scene.getCube().XOY_z_neg,
                    scene.getCube().XOZ_y_pos, scene.getCube().XOZ_y_neg,
                    scene.getCube().YOZ_x_pos, scene.getCube().YOZ_x_neg,
                    scene.getCube().drawSliceBool,
                    scene.getCube().idx, scene.getCube().idy, scene.getCube().idz,
                    scene.getCube().mask, scene.getCube().status,
                    scene.getCube().array3D->head,scene.getCube().array3D->size_x, scene.getCube().array3D->size_y, scene.getCube().array3D->size_z,
                    //traceray
                    this->tmin,this->tmax,this->tinc,
                    this->psp,
                    this->showFrame,this->frameWidth,this->frameColor,
                    this->sample_step,this->absorption_rate,this->transparent_reduce,
                    this->transparentThreshold_low,this->transparentThreshold_high);
    }else{
        //concurrency
    ...
    ui->label_beta->setText(QString("beta=")+QString::number(beta,'f',2));
    ui->label_rho->setText(QString("rho=")+QString::number(rho,'f',2));
    ui->label_rho_2->setText(QString("rho=")+QString::number(rho,'f',2));
}
...
void Widget::keyPressEvent(QKeyEvent *event)
{
    if (event->modifiers() == Qt::CTRL && event->key() == Qt::Key_S)
    {
        ...
        QString filename = QFileDialog::getSaveFileName(this,"Please choose the saving directory",filePathList[filePathList.length()-1],tr("*.bmp;; *.png;; *.jpg;; *.tif;; *.GIF"));
        if(filename.isEmpty())
            return;
        ...
        QMessageBox::information(this,"Reminder","Saving successful!");
    }
}
