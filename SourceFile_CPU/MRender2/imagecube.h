...
public:
    ...
    void dif_shift(double dx, double dy, double dz);
    void shift2Origin();
...
void ImageCube::dif_shift(double dx, double dy, double dz)
{
    this->xshift+=dx; this->yshift+=dy; this->zshift+=dz;
    double x = center.get_x(); double y = center.get_y(); double z = center.get_z();
    center.init(x-dx,y-dy,z-dz);
    //6 cube sides
    XOY_z_pos = center.get_z()+zlength/2;
    XOY_z_neg = center.get_z()-zlength/2;
    XOZ_y_pos = center.get_y()+ylength/2;
